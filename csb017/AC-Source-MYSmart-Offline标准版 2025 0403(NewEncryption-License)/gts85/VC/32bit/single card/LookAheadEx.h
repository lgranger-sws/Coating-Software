...
typedef struct 
{
	short optimizeMode;	     //曲线优化方式
	short blendingType;      //blending类型
	short pad[2];
	double tolerance;	     //轨迹优化误差
	double blendingPrm;      //blending参数
	double blendingMinAngle; //blending的最小角度限制（即当线段向量角度小于该角度时，不做blending，单位：度）
	double blendingMaxAngle; //blending的最大角度限制（即当线段向量角度大于该角度时，不做blending，单位：度）
}TPathOptimizePrm;
...
GT_API GT_SetVelLookAheadModeLa(short crd,int velMode);                     //设置速度前瞻模式
GT_API GT_SetVelSmoothModeLa(int crd,int smoothMode);
...
GT_API GT_SetPathOptimizePrmLa(short crd,short enable,short mode,void *pPrm);						//设置轨迹优化参数
...
GT_API GT_BufSetCrdWorkUnitTagEx(short crd,short workUnitTag,short fifo=0);
...
GT_API GT_BufSetWaitLongVarEx(short crd,short index,long value,short fifo=0);

GT_API GT_BufWaitLongVarEx(short crd,short index,long value,long overTime,short flagMode,long segNum,short fifo);

GT_API GT_BufSetLongVarEx(short crd,short index,long value,short fifo);
GT_API GT_BufSetDoubleVarEx(short crd,short index,double value,short fifo);

GT_API GT_BufWaitWatchVarEx(short crd,TWatchCondition *pWatchCondition,TWaitTimeout *pWaitTimeout,long segNum,short fifo);
GT_API GT_BufStopPosEx(short crd,short profile,TStopPos *pStopPos,short modal,short fifo);
GT_API GT_BufSetSoftLimitEx(short crd,short axis,double positive,double negative,short fifo);
